...
angular.module('ui.directives').directive('uiSelect2', ['ui.config', '$http', function(uiConfig, $http){
	...
	return {
		...
		link: function(scope, elm, attrs, controller) {
			...
			/**
			 * This link function is being triggered twice for the same element, if it is 
			 * loaded from a partial, via a link on the page. If the whole page is loaded with
			 * this partial as a default it only gets triggered once per element.
			 * TODO: Create test case for this and find root of bug.
			 *	
			 * This throws an error in Select2 because the options object we pass through
			 * is empty. To bypass this we silently return if the provided options is undefined.
			 */
			var providedOpts = scope.$eval(attrs.uiSelect2);

			if( providedOpts === undefined ){
				return;
			}

			opts = angular.extend({}, options, providedOpts);
			...
			if (!elm.is('select') && opts.ajax) {
				...
				elm.bind("change", function(){
					var valueToSet = ( typeof(opts.createValue) == "function") ? opts.createValue(elm.val()) : elm.val();
                    var setter = (typeof(opts.valueSetter) == "function") ? opts.valueSetter : controller.$setViewValue;
                    setter( valueToSet );
                    scope.$apply();
				});
			}
