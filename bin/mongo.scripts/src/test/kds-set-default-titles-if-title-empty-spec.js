...
describe("title is empty", function () {
  ...
  it("should set title to [sourceId] - [scoringType]", function () {
    var update = sut.processItem(item);
    expect(update).toEqual({itemId: {_id: '123'}, needsUpdate: true, updates: {"taskInfo.title": '456 - ABC'}});
  });
...
describe("title is not empty", function () {
  ...
  it("should not change a normal title", function () {
    var update = sut.processItem(item);
    expect(update).toEqual({itemId: {_id: '123'}});
  });
});
...
describe("description is empty", function () {
  ...
  it("should set description to [sourceId]", function () {
    var update = sut.processItem(item);
    expect(update).toEqual({itemId: {_id: '123'}, needsUpdate: true, updates: {"taskInfo.description": '456'}});
  });
  ...
  it("should use sourceId from title, if kds.sourceId is not set", function () {
    ...
    expect(update).toEqual({
      itemId: {_id: '123'},
      needsUpdate: true,
      updates: {"taskInfo.description": '789', "taskInfo.extended.kds.sourceId": "789"}
    });
  });
