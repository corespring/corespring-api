...
class ItemFilesTest extends Specification with Mockito{
  ...
  "item files" should {
    ...
    "clone" in {
      ...
      file.storageKey = StoredFile.storageKey(oldId, resource, file)
      ...
      itemFiles.cloneStoredFiles(item) match {
        case Success(updatedItem) => {
          ...
          file.storageKey === StoredFile.storageKey(newId, resource, file)
        }
    ...
    "if clone fails for a file then a list of successful clones are returned" in {
      ...
      goodFile.storageKey = StoredFile.storageKey(oldId, resource, goodFile)
      badFile.storageKey = StoredFile.storageKey(oldId, resource, badFile)
