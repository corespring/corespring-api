...
class LineInteractionTest extends BaseTest{
  ...
  "line interaction 1" should {
    ...
    "be answered correctly" in {
      optli1.flatMap(_.getOutcome(optrd1,ArrayItemResponse("graphTest",Seq("-3,1","-2,3")))).map(_.isCorrect) must beSome(true)
    }
    "be answered correctly with different coordinates" in {
      optli1.flatMap(_.getOutcome(optrd1,ArrayItemResponse("graphTest",Seq("1,9","-4,-1")))).map(_.isCorrect) must beSome(true)
    }
  }
  ...
  "line interaction 3" should {
    ...
    "be answered correctly when slope matches significant figures" in {
      optli3.flatMap(_.getOutcome(optrd3,ArrayItemResponse("graphTest",Seq("1,4.571428571","3,7.714285713")))).map(_.isCorrect) must beSome(true)
    }
  }
