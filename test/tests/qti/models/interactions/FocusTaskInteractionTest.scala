...
class FocusTaskInteractionTest extends Specification {
  ...
  "Focus Task Interaction" should {
    ...
    "selection is correct if checking for selection correctness" in {
      ...
      val response2 = ArrayItemResponse("rid", Seq("A","B","C"), Some(ItemResponseOutcome(0,false, Some("Comment"))))
      ...
      val response3 = ArrayItemResponse("rid", Seq("A","B","D"), Some(ItemResponseOutcome(0,false, Some("Comment"))))
