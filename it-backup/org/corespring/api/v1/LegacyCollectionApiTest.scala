...
import _root_.org.corespring.it.IntegrationSpecification

@deprecated("Move the assertions to CollectionApiIntegrationTest (Work in progress)", "")
class LegacyCollectionApiTest extends IntegrationSpecification {

  /**
   * This is the old v1 test. Will need to be updated to function as an IntegrationTest.
   * So it should be using route() and seeding and tearing down it's data.
   */
  /*
