...
object Build extends sbt.Build {
  ...
  val search = builders.lib("search").settings(
    libraryDependencies ++= Seq(specs2 % "test", playTest % "test", playFramework, elastic4s)
  ).dependsOn(core)
  ...
  val main = play.Project(appName, appVersion, Dependencies.all)
    ...
    .aggregate(qti, public, playerLib, core, apiUtils, commonViews, testLib, clientLogging, search)
    .settings(disableDocsSettings: _*)
