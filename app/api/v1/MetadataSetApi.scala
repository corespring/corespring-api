...
import models.{MetadataSet, Organization, OrganizationService}
...
class MetadataSetApi(metadataSetService:MetadataSetServiceImpl) extends BaseApi {
  ...
  def get(id:ObjectId) = ApiAction{ request =>
    metadataSetService.findOneById(id).map{ m =>
     Ok(m.as[JsValue])
    }.getOrElse(NotFound)
  }
  ...
  def update(id:ObjectId) = ApiAction{ implicit request =>
  ...
  def delete(id:ObjectId) = ApiAction{ request =>
    metadataSetService.delete(request.ctx.organization, id).map{ error =>
      BadRequest(error)
    }.getOrElse(Ok)
  }

  implicit private def seqToJsValue(l:Seq[MetadataSet]) : JsValue = {
