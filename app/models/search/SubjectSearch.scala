 ...
 */
object SubjectSearch extends Searchable{
      ...
      case dbquery:BasicDBObject => {
        dbquery.foldRight[Either[SearchCancelled,MongoDBObject]](Right(MongoDBObject()))((field,result) => result match {
          case Right(searchobj) => field._1 match {
            case Subject.Category => formatStringQuery(field,searchobj)
            case Subject.Subject => formatStringQuery(field,searchobj)
        ...
        dbfields.foldRight[Either[InternalError,SearchFields]](Right(SearchFields(method = method)))((field,result) => {
          result match {
            case Right(searchFields) => field._1 match {
              case Subject.Category => toSearchFieldObj(searchFields,field)
              case Subject.Subject => toSearchFieldObj(searchFields,field)
    ...
    field match {
      ...
      case dbfield:BasicDBObject => {
        if (dbfield.toSeq.size != 1){
          ...
          field._1 match {
            case Subject.Category => formatSortField(field)
            case Subject.Subject => formatSortField(field)
