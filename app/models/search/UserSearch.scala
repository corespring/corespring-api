...
object UserSearch extends Searchable{
      ...
      case dbquery:BasicDBObject => {
        dbquery.foldRight[Either[SearchCancelled,MongoDBObject]](Right(MongoDBObject()))((field,result) => result match {
          case Right(searchobj) => field._1 match {
            case User.userName => formatStringQuery(User.userName,field._2,searchobj)
            case User.fullName => formatStringQuery(User.fullName,field._2,searchobj)
            case User.email => formatStringQuery(User.email,field._2,searchobj)
        ...
        dbfields.foldRight[Either[InternalError,SearchFields]](Right(SearchFields(method = method)))((field,result) => {
          result match {
            case Right(searchFields) => field._1 match {
              case User.userName => toSearchFieldObj(searchFields,field)
              case User.fullName => toSearchFieldObj(searchFields,field)
              case User.email => toSearchFieldObj(searchFields,field)
    ...
    field match {
      ...
      case dbfield:BasicDBObject => {
        if (dbfield.toSeq.size != 1){
          ...
          field._1 match {
            case User.userName => formatSortField(User.userName,field._2)
            case User.fullName => formatSortField(User.fullName,field._2)
            case User.email => formatSortField(User.email,field._2)
