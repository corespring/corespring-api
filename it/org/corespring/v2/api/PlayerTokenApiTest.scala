...
class PlayerTokenApiTest extends IntegrationSpecification {
  ...
  "PlayerTokenApi" should {
    ...
    "create a player token" in new token_createPlayerToken {
      ...
      val settings = Json.stringify(Json.toJson(PlayerAccessSettings.permissiveRead(Json.obj())))
  ...
  trait token_createPlayerToken extends orgWithAccessToken {
    ...
    lazy val result = route(
      FakeRequest(call.method, url, FakeHeaders(), AnyContentAsJson(Json.obj()))).getOrElse(throw new RuntimeException("Couldn't get a result"))
  }
}
