...
class IndexCalculatorIntegrationTest extends IntegrationSpecification {
  ...
  trait createItem
    ...
    lazy val searchResult = Await.result(Global.main.itemIndexService.search(query), 1.second)
  ...
  trait loadJson extends createItem {
    ...
    implicit val nc: Context = Global.main.context
  ...
  "search" should {
    "find an item with an automatically inserted calculator when it's indexed via the app" in new loadJson {
      implicit val itemFormat = Global.main.jsonFormatting.item
    ...
    "find an item with an automatically inserted calculator when index is run manually" in new loadJson {
      ...
      Global.main.db(CollectionNames.item).insert(dbo)
      val cfg = Global.main.elasticSearchConfig
      //Run the indexer
