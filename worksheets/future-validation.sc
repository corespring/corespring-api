//from: https://gist.github.com/timperrett/4662542
...
implicit val F = future.futureInstancesNonblocking(ExecutionContext.Implicits.global)
implicit val M = future.futureMonoid[String]
type ValidationS[T] = Validation[String, T]
val app = Applicative[Future].compose[ValidationS]
def handle(a: Int, b: Int): String = (a+b).toString
val r1: Future[ValidationS[Int]] = Future { Failure.apply("foo") }
val r2: Future[ValidationS[Int]] = Future { Failure.apply("bar") }
...
val output: Future[ValidationS[String]] = app.apply2(r1,r2)(handle)
val o = Await.result(output, 1.second)
val fm = Await.result(inFlatmap, 1.second)
println(s"fm: $fm")
