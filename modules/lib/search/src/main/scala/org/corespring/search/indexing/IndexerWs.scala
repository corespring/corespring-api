 ...
 */
object IndexerWs extends Indexer with JsonUtil {
  ...
  val elasticSearchHost = play.api.Play.current.configuration.
    getString("elasticsearch.webservice.host").getOrElse(throw failedRequirement("elastic search host"))
   ...
   */
  override protected def dropAll() = WS.url(s"http://$elasticSearchHost/_all").delete()
