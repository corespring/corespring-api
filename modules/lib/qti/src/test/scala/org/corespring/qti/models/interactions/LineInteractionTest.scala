...
class LineInteractionTest extends Specification {
  ...
  val qti4 = QtiItem(
    <assessmentItem>
      <itemBody>
        <lineInteraction jsxgraphcore="" responseIdentifier="graphTest" noninteractive=""></lineInteraction>
      </itemBody>
    </assessmentItem>)

  val optli4 = qti4.itemBody.interactions.find(i => i.responseIdentifier == "graphTest" && i.isInstanceOf[LineInteraction]).map(_.asInstanceOf[LineInteraction])

  val optrd4 = qti4.responseDeclarations.find(_.identifier == "graphTest")
  ...
  "line interaction 4" should {
    "be non-interactive" in {
      optli4.map(_.nonInteractive) must beSome(true)
    }
  }
}
