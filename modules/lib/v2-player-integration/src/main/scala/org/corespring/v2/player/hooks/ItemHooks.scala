...
trait DraftItemHooks extends ContainerItemHooks {
  ...
  override def create(maybeJson: Option[JsValue])(implicit header: RequestHeader): Future[Either[StatusMessage, String]] = Future {Left(400,"TODO")}
  ...
  private def savePartOfPlayerDef(itemId: String, json: JsObject)(implicit header: RequestHeader): Future[Either[(Int, String), JsValue]] = Future {
    val out: Validation[V2Error, JsValue] = for {
      draftAndIdentity <- loadDraftAndIdentity(itemId)
