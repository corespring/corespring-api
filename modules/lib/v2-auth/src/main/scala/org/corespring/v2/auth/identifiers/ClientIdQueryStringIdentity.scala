...
trait ClientIdAndOptsQueryStringWithDecrypt extends ClientIdQueryStringIdentity[OrgAndOpts] {
  ...
  override def data(rh: RequestHeader, org: Organization, defaultCollection: ObjectId) =
    OrgAndOpts(org.id, toPlayerOptions(org.id, rh), AuthMode.ClientIdAndOpts)
