...
class ItemDraftServiceTest extends Specification {
  ...
  def assert(isNull: Boolean) = {

    def e(args: Any*): String = (s"${if (isNull) "[loadExpired]" else ""}" +: args).mkString(" ")

    trait scope extends Scope with withMockCollection with ItemDraftDbUtils {

      val _loadExpired = isNull
      ...
      lazy val service = new ItemDraftService {
        override lazy val loadExpired = _loadExpired
        override lazy val collection: MongoCollection = mockCollection
    ...
    e("listForOrg") should {
      ...
      "query by orgId" in new scope {
        ...
        q.value.get("expires") == null must_== isNull
        (q.value - "expires") must_== MongoDBObject("_id.orgId" -> orgId)
      }
    ...
    e("load") should {
      "call collection.findOne" in new scope {
        ...
        q.value.get("expires") == null must_== isNull
        (q.value - "expires") === idToDbo(draftId)
      }
    ...
    e("listByOrgAndVid") should {
      "call collection.find with org and versionedid.id in the query" in new scope {
        ...
        (q.value - "expires") === MongoDBObject("_id.orgId" -> orgId, "_id.itemId" -> itemId)
      }
