...
import com.mongodb.casbah.{ AggregationOptions, MongoCollection }
...
class ItemService(
  ...
  override def countItemsInCollections(collectionIds: ObjectId*): Future[Seq[ItemCount]] = Future {
    ...
    val opts = AggregationOptions(AggregationOptions.CURSOR)
    val output = currentCollection.aggregate(Seq(matchQuery, group), opts) //AggregationOptions(AggregationOptions.CURSOR) )
    val foundCounts = output.toSeq.flatMap(toItemCount)
