...
trait UserService {
   ...
   *
   * Note: If you do not plan to use the UsernamePassword provider just return an empty
   * String in your UserService implementation.
   *
...
object UserService {
  ...
  def createActivation(user: SocialUser): String = {
    delegate.map( _.createActivation(user) ).getOrElse {
      notInitialized()
      ""
    }
  }
  ...
  def activateAccount(uuid: String): Boolean = {
    delegate.map( _.activateAccount(uuid) ).getOrElse {
      notInitialized()
      false
    }
  }
